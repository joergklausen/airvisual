...
try:
    ...
    plot_data(df_x15a, title="KMD HQ Nairobi (AVO X15A)", linestyle=":", path=path)

    df_wich = get_recent_data(wich)
    plot_data(df_wich, title="Mogogosiek Tea Factory (AVO WICH)", linestyle="-.", path=path)

    df_twrg = get_recent_data(twrg)
    plot_data(df_twrg, title="Huduma Center Bomet (AVO TWRG)", linestyle="-", path=path)
except:
    pass
# %%
